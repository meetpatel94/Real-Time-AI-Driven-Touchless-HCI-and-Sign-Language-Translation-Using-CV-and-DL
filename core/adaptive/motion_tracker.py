...
class MotionTracker:
    ...
    def update(self, handedness: str, landmarks, timestamp: Optional[float] = None) -> MotionFeatures:
        ...
        with self._lock:
            ...
            if len(samples) < 2:
                trajectory = tuple((0.0, 0.0) for _ in samples)
                return MotionFeatures(sample_count=len(samples), trajectory=trajectory)

            first_time, first_x, first_y = samples[0]
            trajectory = tuple(
                (center_sample_x - first_x, center_sample_y - first_y)
                for _, center_sample_x, center_sample_y in samples
            )
